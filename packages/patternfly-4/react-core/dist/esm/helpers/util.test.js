...
import { capitalize, getUniqueId, debounce, isElementInView, sideElementIsOutOfView, fillTemplate, pluralize, formatBreakpointMods } from './util';
...
describe('keyHandler works on ApplicationLauncher', () => {
  ...
  const dropdownItems = [React.createElement(DropdownItem, {
    ...
    id: "second",
    component: "button"
  }, "Action 2"), React.createElement(DropdownItem, {
    key: "3",
    id: "third",
    component: "button",
    isDisabled: true
...
test('formatBreakpointMods', () => {
  expect(formatBreakpointMods([{
    modifier: "spacer-none"
  }], styles)).toEqual('pf-m-spacer-none');
  expect(formatBreakpointMods([{
    modifier: "spacer-none",
    breakpoint: 'md'
  }], styles)).toEqual('pf-m-spacer-none-on-md');
  expect(formatBreakpointMods([{
    modifier: "column"
  }, {
    modifier: "row",
    breakpoint: "lg"
  }], styles)).toEqual('pf-m-column pf-m-row-on-lg');
});
